...
func (fabric *Fabric) lay(claim *Claim) {
	...
	for i := claim.topEdge; i < claim.topEdge+claim.height; i++ {
		for j := claim.leftEdge; j < claim.leftEdge+claim.width; j++ {
			...
			if len(fabric.material[i][j]) > 1 {
				for _, claimId := range fabric.material[i][j] {
					fabric.claims[claimId].hasOverlap = true
				}
			}
		}
	}
...
func (fabric *Fabric) firstNonOverlappingClaimId() int {
	for id, claim := range fabric.claims {
		if claim.hasOverlap == false {
...
func (fabric *Fabric) output() {
	for i := 0; i < FabricHeight; i++ {
		for j := 0; j < FabricWidth; j++ {
			...
			if len(cellClaims) == 1 {
				fmt.Printf("%d", fabric.material[i][j][0])
			}
...
func main() {
	...
	fabric.firstNonOverlappingClaimId()
}
